...
export class FCIComponent implements OnInit
{
  ...
  tab3: any
 /////////////////////////////////////////////////////////////////////////////////////end popup model
 constructor(private itraceIt: CrudService, private SpinnerService: NgxSpinnerService, private datepipe: DatePipe) { }
...
const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
 ...
 qrtSubmitF(data) {
   ...
   if (pay_load.action_details.remarks != (null || undefined)) {
...
// addMarkersAndPolyline( imei, vehicle_no) {
//   let icon;
//   for (let i = 0; i < this.trackingData.length; i++) {
//     icon = this.getMarkerIcon(i);

//     const mark = new google.maps.Marker({
//       map: this.map1,
//       position: new google.maps.LatLng(this.trackingData[i].lat, this.trackingData[i].long),
//       title: `${this.trackingData[i].lat}, ${this.trackingData[i].long}`,
//       icon: icon
//     });
...
//     google.maps.event.addListener(mark, 'click', (event) => this.handleMarkerClick(event, this.trackingData[i],vehicle_no,imei));

//     let infowindowMarker = new google.maps.InfoWindow({ content: this.contentsInfo });
//   }
...
generateCustomerInfo(customer): string {
  ...
  return `<table class="border">
    <tbody>
      <tr><td>Location</td><td>:</td><td>${customer.LocationCode}</td></tr>
      <tr><td>PodStatus</td><td>:</td><td>${pod}</td></tr>
      <tr><td>Type</td><td>:</td><td>${type}</td></tr>
      <tr><td>ArrivalTime</td><td>:</td><td>${arrival_time}</td></tr>
      <tr><td>DepartureTime</td><td>:</td><td>${departure_time}</td></tr>
    </tbody>
  </table>`;
}
