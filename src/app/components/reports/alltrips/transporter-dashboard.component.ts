...
export class TransporterDashboardComponent implements OnInit {
  ...
  Driver_documentList: any = [
...

    {
      ...
      img: 'assets/imagesnew/Icon/DrivingLicense.png',
    },
  ...
  onAccept(): void {
    const selectedItems: any = this.filterData.filter(
      (item, ind) =>
        item?.Raised_By === 'customer' && this.isSelected('checkbox-' + ind)
    );
  ...
  onReject(): void {
    const selectedItems = this.filterData.filter(
      (item, ind) =>
        item?.Raised_By === 'customer' && this.isSelected('checkbox-' + ind)
    );
  ...
  patchSafetyFeatures(index: number) {
    ...
    const safetyFeaturesArray = (
      this.getinvoice().controls[index] as FormGroup
    ).get('VehicleDocument') as FormArray;

    safetyFeaturesArray.patchValue(
      this.Vehiclefull_information[index]?.documents
    );
    const DriverDocument = (this.getinvoice().controls[index] as FormGroup).get(
      'DriverDocument'
    ) as FormArray;
  ...
  addSafetyFeature(idx: number, safetyFeature: string) {
    const safetyFeatures =
      this.Vehiclefull_information[idx]?.vehicle_data?.safety_features;

    // Initialize FormArray for safetyFeatures
    const safetyFeaturesArray = this.getinvoice()
      .at(idx)
      .get('safetyFeatures') as FormArray;
  ...
  DateselectGroup() {
    this.commonDateSelectFn('#quoteDateStart')
    this.commonDateSelectFn('#quoteDateEnd')
  }

  commonDateSelectFn(id) {
    console.log(id);
    $(document).ready(() => {
      $(id)
        .datepicker({
  ...
  orderstatus_Decline(status) {
    if (confirm('Are you sure you want to decline this?')) {
      ...
      this.service.orderstatus(formdata).subscribe((data: any) => {
        if (data.Status == 'sucess') {
          alert(data.Message);
          this.filterSubmit(this.store_filtersubmit_data, status);
        } else {
          alert(data.Message);
        }
      });
          ...
          this.service.orderdata(formdata).subscribe((data: any) => {
            if (data.Status == 'sucess') {
              ...
              if (status == 1) {
                ...
                this.searchedQuoteData = data.Data;
                this.quoteData_assign();
              }
            } else {
              this.SpinnerService.hide('spinner-5');
              alert(data.Message);
  ...
  requiresExpiryDate(): boolean {
    //
    return !(
      this.document_for === 'Aadhaar Card' ||
      this.document_for === 'Pan Card' ||
      this.document_for === 'Photograph'
    );
  }
  ...
  Get_vehicle(vehicle_id, index) {
    ...
    this.service.vehicledetails(formData).subscribe((res: any) => {
      if (res.Status == 'sucess') {
        ...
        const selectedVehicle = this.Vehicle_list.find(
          ...
          if (formGroup) {
            ...
            if (capacityControl) {
              capacityControl.setValue(
                this.Vehiclefull_information?.vehicle_data
            ...
            if (capacityControl) {
              capacityControl.setValue(
                this.Vehiclefull_information?.vehicle_data?.vehicle_type
              );
            }
          }
        }
        ...
        const safetyFeaturesArray = (
          this.getinvoice().controls[index] as FormGroup
        ).get('Capacity') as FormArray;
        safetyFeaturesArray.patchValue(
          this.Vehiclefull_information[index]?.vehicle_data
            ?.vehicle_capacity_tons
        );
        ...
        DriverDocument.patchValue(
          this.Vehiclefull_information[index]?.vehicle_data?.vehicle_type
        );
      } else {
        this.SpinnerService.hide('spniiner-1');
      }
  ...
  get_driver(driver_id, index) {
    ...
    this.service.driverdetails(formData).subscribe((res: any) => {
      if (res.Status == 'sucess') {
        ...
        var store: any = [];
        for (const [key, value] of Object.entries(
          this.driverfull_information[index]?.documents
        )) {
          store.push(key);
        }
        this.info_store_driver = store;

        const selectedVehicle = this.Driver_list.find(
          (v) => v.id === driver_id
        );
        ...
        if (selectedVehicle) {
          ...
          if (formGroup) {
            ...
            if (capacityControl) {
              capacityControl.setValue(
                this.driverfull_information?.driver_data?.mob_no
              );
            }
            // const capacityControl1 = formGroup.get('Type');
            // if (capacityControl) {
        ...
        DriverDocument.patchValue(
          this.driverfull_information[index]?.driver_data?.mob_no
        );
      } else {
        this.SpinnerService.hide('spniiner-1');
      }
  ...
  isCheckboxInvalid(index: number, formname: string): boolean {
    ...
    return (
      ...
      (fieldControl?.dirty || fieldControl?.touched || this.submite)
    );
  }
  ...
  onTabClicked(tab) {
    ...
    if (tab == 'driver' && !Object.keys(this.driverDashboardData).length) {
      ...
      tab == 'billing' &&
      (this.vehicleList.length == 0 || this.customers.length == 0)
    ) {
